...
def test_bge_m3_embedding(sentence_transformers_server: Server) -> None:
    sentence_transformers_server.model_manager.load_huggingface_model(
        BGE_M3_MODEL, BACKEND_ID_SENTENCE_TRANSFORMERS, "feature-extraction", skip_if_loaded=True
    )
...
def test_bge_m3_embedding_multiple(sentence_transformers_server: Server) -> None:
    sentence_transformers_server.model_manager.load_huggingface_model(
        BGE_M3_MODEL, BACKEND_ID_SENTENCE_TRANSFORMERS, "feature-extraction", skip_if_loaded=True
    )
